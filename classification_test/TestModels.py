...
df = pd.DataFrame(dataloaders["test"].dataset.samples)
...
plt.xlabel('Pattern')
plt.ylabel('Count [n]')
plt.savefig('DataBase_' + 'test' + '.jpg')
plt.show()
plt.close()

#%% define the models to load
model_list = ['/home/cw9/sds_hd/sd18a006/Marlen/datasets/stainNormalization/stainGAN_camelyon16/trainedModels',
            '/home/cw9/sds_hd/sd18a006/Marlen/datasets/stainNormalization/patchCamelyon/trainedModels'
]
...
n = 0
df_values = pd.DataFrame(list(range(0,len(dataloaders['test'].sampler.data_source.imgs))))
...
for imodel in model_list:
    ...
    model_ft = torch.load(imodel + '/model_ResNet152.pt')
    model_ft.eval()
    vector_prd = []
    vector_exp = []
    #del(outputs_)
    ...
    plt.ylabel('Pattern predicted by model #' + str(n))
    plt.savefig('ConfMat_' + str(n) + '.jpg')
    plt.show()
    plt.close()
